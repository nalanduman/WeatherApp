...
class WeatherDataService: BaseService, WeatherDataServiceProtocol {
    ...
    func getWeatherList(params: Parameters, completion: @escaping CallbackResponse<WeatherList>) {
        ...
        parameters[Constants.ParameterKeys.appID] = appID
        parameters[Constants.ParameterKeys.units] = "imperial"
        parameters[Constants.ParameterKeys.cnt] = 40
        dataRequest.params = parameters
        
        networking.request(with: dataRequest.getUrlRequest(with: ApiEndpoint.forecast.rawValue), showLogs: true) { (result: Result<WeatherList>) in
            switch result {
            case .success(let response):
                completion(response)
            case .failure(_):
